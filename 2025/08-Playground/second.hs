...
compute :: Input -> Int
compute input = fromJust $ fst $ L.foldl' connect (Nothing, map (\p -> [p]) input) pairings
  where
    connect :: (Maybe Int, [Circuit]) -> Pairing -> (Maybe Int, [Circuit])
    connect a@(Just _, _) _ = a
    connect a@(Nothing, acc) (p1@(Point x1 _ _), p2@(Point x2 _ _)) | connected = a
                                                                    | length connect == 1 = (Just (x1 * x2), [])
                                                                    | otherwise = (Nothing, connect)
      where
